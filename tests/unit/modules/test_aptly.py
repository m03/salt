...
class AptlyTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_set_repo(self):
        '''
        Test - Configure settings for the local package repository.
        '''
        kwargs = {
            'name': 'devtest',
            'comment': 'Packages for the devtest project',
        ...
        old_repo = copy.deepcopy(REPO)
        old_repo['comment'] = 'Old comment'
        cmd_output = 'Local repo [{}]: {} successfully updated.\n'.format(kwargs['name'],
                                                                          kwargs['comment'])
        mock_get_repo = MagicMock(side_effect=[old_repo, REPO])
        with patch.dict(aptly.__salt__, {'aptly.get_repo': mock_get_repo}), \
            patch('salt.modules.aptly._cmd_run',
                  MagicMock(return_value=cmd_output)), \
            patch('salt.modules.aptly._validate_config',
                  MagicMock(return_value=None)):
            self.assertTrue(aptly.set_repo(**kwargs))
